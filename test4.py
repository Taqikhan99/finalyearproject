import face_recognition
import cv2,multiprocessing,time
import numpy as np
import time,os
from pathlib import Path
from crdoperations import UserIdsGetting
from training import names,encodings,trainingImages
# from dataset import DatasetGenerator
from dbconnection import DbConnection
from crdoperations import UserInsertion
# datbase connection
conn=DbConnection()
conn.connectToDb()

# getting userIds
users=UserIdsGetting()
userids=users.gettingUserId(conn.cursor)

print(userids)

import tensorflow as tf
print(tf.config.list_physical_devices('GPU'))
  
# Initialize some variables
face_locations = []
face_encodings = []
face_names = []


class FPS:
    prevTime=0
    newTime=0
    def fpsCalculate(self):
        
        
        newTime=time.time()
        
        fps=1/(newTime-FPS.prevTime)
        FPS.prevTime=newTime
        fps=str(int(fps))
        return fps






def resizeImg(img):
        resize=cv2.resize(img,(200,200))
        return resize

def generateDataset(img, userid,imgid):
        
        # creating a folder for a user
        Path("images/user_{}".format(userid)).mkdir(parents=True,exist_ok=True)
        # gray=self.convertToGray(img)
        # writing images in the user folder
        try:
            time.sleep(0.01)    
               
            img=resizeImg(img)   
            savingPath="images/user_"+str(userid)+"/user_"+str(userid)+"."+str(imgid)+".jpg"
            savingPath2="images/user_"+str(userid)+"/user_"+str(userid)+"."+str(imgid+1)+".jpg"
            cv2.flip(img,1)
            cv2.imwrite(savingPath,img)
            cv2.imwrite(savingPath2,img)
   
        except:
            print('Check please')
        print("Images Saved successfully!")


def cameraSetup(fpsObject,userInsertObj):
    count=1
    video_capture = cv2.VideoCapture(1)
    process_this_frame = True
    while True:
        # Grab a single frame of video
        ret, frame = video_capture.read()
        fps=fpsObject.fpsCalculate()
        cv2.putText(frame,fps, (7, 70),cv2.FONT_HERSHEY_COMPLEX,1, (100, 255, 0),2, cv2.LINE_AA)
        # Resize frame of video to 1/4 size for faster face recognition processing
        small_frame = cv2.resize(frame, (0, 0), fx=0.25, fy=0.25)

        # Convert the image from BGR color (which OpenCV uses) to RGB color (which face_recognition uses)
        rgb_small_frame = small_frame[:, :, ::-1]

        # Only process every other frame of video to save time
        if process_this_frame:
            # Find all the faces and face encodings in the current frame of video
            face_locations = face_recognition.face_locations(rgb_small_frame,number_of_times_to_upsample=2)
            face_encodings = face_recognition.face_encodings(rgb_small_frame, face_locations,num_jitters=1)

            face_names = []
            for face_encoding in face_encodings:
                # See if the face is a match for the known face(s)
                matches = face_recognition.compare_faces(encodings, face_encoding,tolerance=0.6)
                name = "Unknown"

                # # If a match was found in known_face_encodings, just use the first one.
                # if True in matches:
                #     first_match_index = matches.index(True)
                #     name = known_face_names[first_match_index]

                # Or instead, use the known face with the smallest distance to the new face
                face_distances = face_recognition.face_distance(encodings, face_encoding)
                best_match_index = np.argmin(face_distances)
                if matches[best_match_index]:
                    name = names[best_match_index]

                face_names.append(name)

        process_this_frame = not process_this_frame


        # Display the results
        for (top, right, bottom, left), name in zip(face_locations, face_names):
            # Scale back up face locations since the frame we detected in was scaled to 1/4 size
            top *= 4
            right *= 4
            bottom *= 4
            left *= 4

            # Draw a box around the face
            

            # Draw a label with a name below the face
            cv2.rectangle(frame, (left, bottom), (right, bottom+15), (0, 0, 255), cv2.FILLED)
            font = cv2.FONT_HERSHEY_DUPLEX
            cv2.putText(frame, name, (left + 6, bottom+10), font, 0.5, (255, 255, 255), 1)
            if name=="Unknown":

                

                cv2.rectangle(frame, (left-10, top-10), (right+10, bottom+10), (0, 0, 255), 1)
                # th = multiprocessing.Process(target=generateDataset,args=[(frame[top:bottom,left:right],4)])
                # global processes
                # processes.append(th)
                
                if count<6:
                    # wait for 1 sec
                    
                    generateDataset(frame[top:bottom,left:right],userids[-1]+1,count)
                    print(count)
                if count<=1:
                    time.sleep(3)
                    userInsertObj.insertUser(conn.cursor,userids[-1]+1)
                    print("Saved user to database")
                    # calling training method again
                    time.sleep(5)
                    trainingImages()
                    time.sleep(0.5)
                    print("done retraining")
                    

                count+=1
                
            else:
                cv2.rectangle(frame, (left-1, top), (right+1, bottom), (0, 0, 255), 1)
        
              
        # Display the resulting image
        cv2.imshow('Video', frame)

        # Hit 'q' on the keyboard to quit!
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break

    # Release handle to the webcam
    video_capture.release()
    cv2.destroyAllWindows()


# main area



# making fps obj
trainingImages()
fpsObject=FPS()
# calling insert obj

userInsertObj=UserInsertion()
cameraSetup(fpsObject,userInsertObj)






























